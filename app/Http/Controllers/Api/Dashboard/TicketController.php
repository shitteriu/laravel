...
class TicketController extends Controller
{
    ...
    public function quickActions(Request $request): JsonResponse
    {
        ...
        if ($action === 'status') {
            $tickets->update(['status_id' => $request->get('value')]);
            return response()->json(['message' => __('The Status of the selected tickets has been changed')]);
        }
        if ($action === 'delete') {
            foreach ($tickets->get() as $ticket) {
                /** @var Ticket $ticket */
    ...
    public function ticketQuickActions(Ticket $ticket, Request $request): JsonResponse
    {
        ...
        if ($action === 'status') {
            $ticket->status = $value;
            $ticket->saveOrFail();
            return response()->json(['message' => __('Ticket assigned to the selected status'), 'ticket' => new TicketManageResource($ticket), 'access' => $ticket->verifyUser($user)]);
        }
        return response()->json(['message' => __('Status Cuack')], 404);
